...
function getTodoChipStyle(todo, dateStr) {
...
function CalendarView({ todos, setTodos }) {
  ...
  // For each date-type todo, compute grid span positions
            ...
            return (
              <div key={weekIdx} className="month-week-row">
                ...
                {weekSpans.map(({ todo, cellIndex, spanLen, isFirstCell }) => {
                  ...
                  const done = firstDateOfSpanInWeek
                    ? isCompletedOnDate(todo, formatDateStr(firstDateOfSpanInWeek))
                    : false

                  return (
                    <div
                      key={`${todo.id}-${cellIndex}`}
                      className="month-span-bar"
                      style={{
        ...
        <div className="week-view">
          ...
          <div className="day-checklist">
            ...
            {selectedDayTodos.length === 0 ? (
              <p className="empty-text" style={{ marginTop: '1rem' }}>이 날은 할 일이 없어요.</p>
            ) : (
